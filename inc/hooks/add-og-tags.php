...
function add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'easy-journal' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'easy-journal' ) : esc_html__( 'tagged with', 'easy-journal' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'easy-journal' ), $specify, $term_name );
	...
	if ( is_search() ) {
		...
		$card_long_description = sprintf( esc_html__( 'Search results for %s.', 'easy-journal' ), $search_term );
		$card_description      = $card_long_description;
	}
